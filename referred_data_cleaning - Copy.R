...
#set x_referred value to 1
df_clean$X_REFERRED <- 1
...
#create list of disk and mic columns
mic_cols <- colnames(df_clean)[grepl("_NM", colnames(df_clean))]
disk_cols <- colnames(df_clean)[grep("_ND", colnames(df_clean))]


# Separate operators and values
df_clean <- df_clean %>%
  mutate(across(all_of(mic_cols), ~ str_extract(., "[<=>]+"), .names = "{col}_op")) %>%
  mutate(across(all_of(mic_cols), ~ str_replace(., "[<=>]+", ""))) %>%
  mutate(across(all_of(disk_cols), ~ str_extract(., "[<=>]+"), .names = "{col}_op")) %>%
  mutate(across(all_of(disk_cols), ~ str_replace(., "[<=>]+", "")))



# Apply as.disk and as.mic
df_clean <- df_clean %>%
  mutate(across(all_of(mic_cols), as.disk)) %>%
  mutate(across(all_of(disk_cols), as.mic))


# Convert factor columns to character
df_clean <- df_clean %>% mutate(across(where(is.factor), as.character))

# Replace NA with ''
df_clean[is.na(df_clean)] <- ''


# Recombine operators and cleaned values
df_clean <- df_clean %>%
  mutate(across(all_of(mic_cols), ~ ifelse(is.na(.), NA, paste0(get(paste0(cur_column(), "_op")), .)))) %>%
  mutate(across(all_of(disk_cols), ~ ifelse(is.na(.), NA, paste0(get(paste0(cur_column(), "_op")), .))))


# Identify columns with "_op" in their names
op_cols <- grep("_op", colnames(df_clean))

# Remove those columns
df_clean <- df_clean[, -op_cols]
...
pheno_cols <- c('AMPC','ESBL','CARBAPENEM','MBL','BETA_LACT','X_MECA','INDUC_CLI','MRSA')
